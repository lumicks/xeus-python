...
raw_interpreter::raw_interpreter(bool redirect_output_enabled /*=true*/,
...
void raw_interpreter::configure_impl() {
    ...
    jedi.attr("api").attr("environment").attr("get_default_environment") = py::cpp_function(
        [jedi]() { jedi.attr("api").attr("environment").attr("SameEnvironment")(); });
...
void raw_interpreter::execute_request_impl(send_reply_callback cb, int execution_count,
    ...
    try {
        ...
        if (py::isinstance(last_stmt, ast.attr("Expr"))) {
            ...
            py::object compiled_interactive_code =
                builtins.attr("compile")(interactive_ast, filename, "single");
        ...
        if (error.m_ename == "SyntaxError") {
            if (code.find("%") != std::string::npos) {
                error.m_traceback.push_back(
                    "There may be Ipython magics in your code, this feature is not supported in "
                    "xeus-python raw mode! Please consider switching to xeus-python normal mode or "
                    "removing these magics");
            }
        }
...
nl::json raw_interpreter::complete_request_impl(const std::string& code, int cursor_pos) {
    py::gil_scoped_acquire acquire;
    nl::json kernel_res;
    ...
    if (py::len(completions) != 0) {
        cursor_start -= py::len(completions[0].attr("name_with_symbols"))
                        - py::len(completions[0].attr("complete"));
...
nl::json raw_interpreter::inspect_request_impl(const std::string& code, int cursor_pos,
                                               int /*detail_level*/) {
    py::gil_scoped_acquire acquire;
    nl::json kernel_res;
    nl::json pub_data;
