...
xeus_logger_client::xeus_logger_client(xeus::xcontext& context, const std::string& user_name,
                                       const xeus::xconfiguration& config,
                                       const std::string& file_name)
    : m_user_name(user_name), m_file_name(file_name), m_session_id(xeus::new_xguid()),
...
void xeus_logger_client::register_kernel_status_listener() {
    p_client->register_kernel_status_listener(
        [this](bool status) { handle_kernel_status_message(status); });
}

void xeus_logger_client::handle_kernel_status_message(bool status) { kernel_dead = status; }
...
void xeus_logger_client::send_on_shell(const std::string& msg_type, nl::json content) {
    ...
    xeus::xmessage msg(xeus::xmessage::guid_list(), std::move(header), nl::json::object(),
                       nl::json::object(), std::move(content), xeus::buffer_sequence());
...
std::size_t xeus_logger_client::iopub_queue_size() const { return p_client->iopub_queue_size(); }
...
void xeus_logger_client::start() { p_client->start(); }

void xeus_logger_client::stop_channels() { p_client->stop_channels(); }
